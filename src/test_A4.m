...
% folder.
acc = [];
userList = [1 7 9 13 16 19 20 22 23 26 27 31 32 34];
gest = A3.gestList(1);

% Get Training and Test data for a particular user
% Change the user below to get result for a different user.
user = 1
[train, test] = A4.getClassData(gest(1), userList(user), 80);
[~,m] = size(train);
...
for i = 1:1
    gest = A4.gestList(1);
    [train, test] = A4.getClassData(gest(i), user, 80);
    [~,m] = size(train);
